...
33. [build.gradle.kts 의존성 해설 문서](#index33)
...
### 33. build.gradle.kts 의존성 해설 문서
<a id="index33"></a>

`docs` 폴더에는 `build.gradle.kts`에 들어 있는 플러그인, BOM, 의존성, 테스트 의존성을 따로 풀어서 설명한 문서들이 있다.

README는 스프링부트 전체 흐름을 보는 곳이고, `docs`는 의존성을 하나씩 뜯어보는 곳이라고 생각하면 된다.
...
├ dependencies
│  └ 의존성별 사용 예제 문서
├ build-gradle-overview.md
├ main-dependencies.md
├ development-runtime-processor-dependencies.md
└ test-dependencies.md
```

0. [의존성별 사용 예제 문서](./docs/dependencies/README.md)  
   `build.gradle.kts`에 들어 있는 의존성을 1개 파일당 1개씩 설명해둔 문서다.

1. [빌드 설정 전체 구조](./docs/build-gradle-overview.md)  
   Gradle 플러그인, Java 버전, Repository, BOM, Gradle task 설정을 설명한다.

2. [main 구현 의존성](./docs/main-dependencies.md)  
   `implementation(...)` 중심으로 웹, DB, 보안, GraphQL, Vaadin, Modulith 의존성을 설명한다.

3. [개발/런타임/컴파일 보조 의존성](./docs/development-runtime-processor-dependencies.md)  
   `developmentOnly`, `compileOnly`, `runtimeOnly`, `annotationProcessor`가 무엇인지 설명한다.

4. [테스트 의존성](./docs/test-dependencies.md)  
   `testImplementation`, `testCompileOnly`, `testRuntimeOnly`, `testAnnotationProcessor`를 설명한다.
...
현재 프로젝트는 학습용으로 많은 Starter가 들어 있다.
여러 기능을 한 번에 볼 수 있다는 장점은 있지만, 실제 서비스 프로젝트에서는 필요한 의존성만 남기는 것이 좋다.

특히 JDBC, Session JDBC, MySQL 관련 의존성이 있으므로 DataSource 설정이 없으면 애플리케이션 실행이나 테스트에서 DB 설정 오류가 날 수 있다.
