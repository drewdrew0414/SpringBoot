# testImplementation - spring-boot-starter-session-data-redis-test
...
이 의존성은 spring-boot-starter-session-data-redis-test 기능을 테스트 코드에서 검증하기 위한 테스트 전용 의존성이다.
...
테스트 Starter가 많아질수록 테스트 컨텍스트가 무거워질 수 있으므로 필요한 것만 남기는 편이 좋다.
...
- artifact: spring-boot-starter-session-data-redis-test
