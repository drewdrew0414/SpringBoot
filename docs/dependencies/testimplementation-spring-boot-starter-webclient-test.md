# testImplementation - spring-boot-starter-webclient-test
...
이 의존성은 spring-boot-starter-webclient-test 기능을 테스트 코드에서 검증하기 위한 테스트 전용 의존성이다.
...
class ReactiveApiTest {
    ...
    @Test
    void call() {
        webTestClient.get().uri("/reactive/hello")
                .exchange()
                .expectStatus().isOk();
    }
}
...
- artifact: spring-boot-starter-webclient-test
