# implementation - spring-modulith-starter-core
...
하나의 애플리케이션 안에서 모듈 경계를 관리하는 Spring Modulith 의존성이다.
...
Spring Modulith는 마이크로서비스로 쪼개기 전, 하나의 애플리케이션 안에서 모듈 경계를 명확하게 만들도록 돕는다. 패키지 간 의존 관계를 검증하고 이벤트 기반 모듈 통신을 구조화할 수 있다.
...
record OrderCompletedEvent(Long orderId) {}
~~~
...
- artifact: spring-modulith-starter-core
